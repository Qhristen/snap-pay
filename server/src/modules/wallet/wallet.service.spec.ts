...
describe('WalletService', () => {
  ...
  const getMockWallet = (id = 'wallet-1', userId = 'user-1') => ({ id, userId, balance: '100.00', currency: 'USD' });
  const mockTransaction = { id: 'txn-1', amount: '50.00', createdAt: new Date() };
  ...
  describe('withdraw', () => {
    it('should throw BadRequestException if balance is insufficient', async () => {
      ...
      await expect(service.withdraw(mockUser as any, { amount: 150 }))
  ...
  describe('transfer', () => {
    it('should throw BadRequestException if transferring to self', async () => {
      await expect(service.transfer(mockUser as any, { recipientId: 'user-1', amount: 50 }))
        .rejects.toThrow(BadRequestException);
    });
    ...
    it('should successfully transfer funds', async () => {
      ...
      const result = await service.transfer(mockUser as any, { recipientId: 'user-2', amount: 50 });
