...
export class WithdrawalProcessor extends WorkerHost {
  ...
  async process(job: Job<{ withdrawalId: string; userId: string; amount: number }>) {
    ...
    try {
      ...
      if (transaction.status !== TransactionStatus.PENDING) {
        this.logger.warn(`Transaction ${withdrawalId} is already ${transaction.status}`);
        return;
      }
      ...
      if (isSuccess) {
        ...
        this.gatewayService.emitTransferNotification(userId, 'System', new Decimal(amount).dividedBy(100).toFixed(2), withdrawalId);
      } else {
        ...
        await queryRunner.commitTransaction();
        this.logger.error({ event: 'withdrawal_failed_and_refunded', withdrawalId });
        
        const balanceNaira = new Decimal(wallet?.balance || 0).dividedBy(100).toFixed(2);
        this.gatewayService.emitBalanceUpdate(userId, balanceNaira, wallet?.currency || 'NGN');
      }

    } catch (error) {
      await queryRunner.rollbackTransaction();
      this.logger.error({ event: 'withdrawal_processor_error', withdrawalId, error: error.message });
      throw error; // Retry
    } finally {
