...
export class WalletProcessor extends WorkerHost {
  ...
  async process(job: Job<any>) {
    ...
    this.logger.log(`[Job] Starting wallet update processing: ${job.name} for user: ${userId}`);
    ...
    switch (job.name) {
      ...
      case 'transfer-notification': {
        const { fromUsername, amount, transactionId } = job.data as TransferNotificationData;
        this.gatewayService.emitTransferNotification(userId, fromUsername, amount, transactionId);
        this.logger.log(`[Socket] Transfer notification emitted for user: ${userId}`);
        break;
      }
      default:
