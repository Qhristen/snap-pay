  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-12 space-y-4">
        ...
        <p className="text-xs text-white/40 animate-pulse">Loading notifications...</p>
      </div>
    );
  }
    ...
    <div className={cn("flex flex-col h-full", className)}>
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[10px] font-bold text-white uppercase tracking-widest">Alerts</h3>
        </div>
        {unreadCount > 0 && (
          <button 
            onClick={() => markAllAsRead()}
            className="flex items-center gap-1.5 text-[10px] font-bold text-primary hover:text-white transition-colors uppercase tracking-tight group"
          >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar max-h-[400px]">
        {notifications?.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 px-8 text-center space-y-3">
            <div className="h-12 w-12 bg-white/5 flex items-center justify-center rounded-2xl">
              <Bell size={20} className="text-white/20" />
            </div>
            <div>
              <p className="text-sm font-bold text-white/60">No notifications</p>
          ...
          <div className="divide-y divide-white/5">
            {notifications?.map((n) => (
              <div
                ...
                className={cn(
                  "p-5 cursor-pointer transition-all duration-300 hover:bg-white/[0.02] relative group",
              ...
              >
                ...
                <div className="flex justify-between items-start gap-4 mb-1.5">
                  <p className={cn(
                    ...
                    !n.isRead ? "text-primary" : "text-white/60 group-hover:text-white/80"
                  )}>
                    {n.title}
                  </p>
                  <span className="text-[9px] font-medium text-white/20 whitespace-nowrap pt-0.5 uppercase">
                    {(() => {
                ...
                <p className="text-[11px] text-white/40 group-hover:text-white/50 leading-relaxed line-clamp-2">
                  {n.message}
                </p>
              </div>
            ))}
